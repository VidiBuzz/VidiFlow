...
function card(ch, idx) {
    const n = Math.round(ch.order ?? 0);
    const isBonus = ch.id.includes('b') || ch.id.includes('v');
    const dn = ch.id === 'foreword' ? 'Foreword' : 'Chapter ' + n;
    ...
    const bonusPill = isBonus ? '<span class="chapter-pill bonus-pill">Bonus</span>' : '';
    const delay = Math.min((idx % 12) * 40, 480);
    return '<a class="chapter-card' + (isBonus ? ' bonus-card' : '') + '" style="animation-delay:' + delay + 'ms" href="print-book.html?chapter=' + ch.id + '">' +
        '<div class="chapter-number">' + esc(dn) + '</div>' +
        '<div class="chapter-title">' + esc(t) + '</div>' +
        '<div class="chapter-desc">' + esc(s) + '</div>' +
        '<div class="chapter-meta">' + bonusPill + '<span class="chapter-pill">' + esc(ch.readTime || '10') + ' min read</span></div>' +
        '</a>';
}
...
sorted.forEach(([pn, arr]) => {
    ...
    sections += '<div class="part-section" id="' + id + '">' +
        ...
        '<div class="chapter-grid">' + arr.map((c, i) => card(c, i)).join('') + '</div></div>';
});
...
const CSS = '<style>:root{--fire:#FF6B35;--gold:#C8A951;--bg:#060914;--bg-card:rgba(255,255,255,.04);--border:rgba(255,255,255,.09);--border-bright:rgba(255,255,255,.18);--accent-glow:rgba(255,107,53,.25);--card-shadow:0 1px 0 rgba(255,255,255,.04) inset,0 8px 24px rgba(0,0,0,.25)}*,*::before,*::after{margin:0;padding:0;box-sizing:border-box}html{scroll-behavior:smooth}body{font-family:"DM Sans",sans-serif;background:var(--bg);color:#E2E8F0;overflow-x:hidden}.container{position:relative;z-index:1;max-width:1100px;margin:0 auto;padding:3rem 2rem 5rem}.back-link{display:inline-flex;align-items:center;gap:.5rem;color:var(--gold);text-decoration:none;font-size:.85rem;font-weight:600;letter-spacing:1px;margin-bottom:2rem;transition:color .2s}.back-link:hover{color:#fff}.page-header{text-align:center;margin-bottom:3rem}.page-eyebrow{font-size:.7rem;font-weight:700;letter-spacing:4px;text-transform:uppercase;color:var(--fire);margin-bottom:.5rem}.page-title{font-family:"Bebas Neue",sans-serif;font-size:clamp(2.5rem,6vw,4.5rem);letter-spacing:2px;color:#fff;line-height:1;margin-bottom:.5rem}.page-subtitle{font-family:"DM Serif Display",serif;font-style:italic;color:rgba(255,255,255,.5);font-size:1.1rem;margin-bottom:.5rem}.page-stats{display:flex;justify-content:center;gap:2rem;margin-top:1rem}.page-stat{text-align:center}.page-stat-num{font-family:"Bebas Neue",sans-serif;font-size:2rem;color:var(--fire);line-height:1}.page-stat-label{font-size:.65rem;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,.35)}.part-nav{display:flex;flex-wrap:wrap;justify-content:center;gap:.4rem;margin-bottom:2.5rem;position:sticky;top:60px;z-index:10;background:rgba(6,9,20,.92);backdrop-filter:blur(16px);padding:.75rem 1rem;border-radius:8px;border:1px solid var(--border)}.part-nav a{font-size:.72rem;font-weight:700;letter-spacing:1px;color:rgba(255,255,255,.5);text-decoration:none;padding:.35rem .75rem;border-radius:4px;transition:all .2s;white-space:nowrap}.part-nav a:hover{color:#fff;background:rgba(255,107,53,.15)}.part-section{margin-bottom:2.5rem;scroll-margin-top:120px}.part-header{display:flex;align-items:baseline;gap:.75rem;margin-bottom:1.25rem;padding-bottom:.5rem;border-bottom:1px solid var(--border)}.part-num{font-family:"Bebas Neue",sans-serif;font-size:2.5rem;color:var(--fire);line-height:1;letter-spacing:1px}.part-title{font-family:"Kumbh Sans",sans-serif;font-size:1.4rem;font-weight:800;color:#fff}.part-chapter-count{font-size:.7rem;color:rgba(255,255,255,.3);letter-spacing:1px;margin-left:auto}.chapter-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:1.25rem}.chapter-card{position:relative;background:var(--bg-card);border:1px solid var(--border);border-radius:12px;padding:1.25rem 1.4rem 1.25rem 1.6rem;cursor:pointer;text-decoration:none;display:block;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);box-shadow:var(--card-shadow);overflow:hidden;opacity:0;transform:translateY(14px);animation:cardFadeUp .6s cubic-bezier(.2,.7,.2,1) forwards;transition:transform .25s ease,box-shadow .25s ease,border-color .3s ease,background .25s ease}.chapter-card::before{content:"";position:absolute;left:0;top:0;bottom:0;width:4px;background:linear-gradient(180deg,var(--fire),var(--gold));opacity:.5;transition:opacity .3s ease,box-shadow .3s ease,width .3s ease}.chapter-card:hover{transform:translateY(-3px) scale(1.012);border-color:var(--border-bright);background:rgba(255,255,255,.06);box-shadow:0 12px 36px rgba(0,0,0,.35),0 0 0 1px rgba(255,107,53,.15) inset}.chapter-card:hover::before{opacity:1;width:6px;box-shadow:0 0 14px var(--accent-glow)}.chapter-card:focus-visible{outline:2px solid var(--fire);outline-offset:3px}@keyframes cardFadeUp{0%{opacity:0;transform:translateY(14px)}100%{opacity:1;transform:translateY(0)}}.chapter-number{font-size:.55rem;font-weight:800;letter-spacing:2px;text-transform:uppercase;color:var(--fire);margin-bottom:.25rem}.chapter-title{font-family:"Kumbh Sans",sans-serif;font-size:.9rem;font-weight:700;color:#fff;margin-bottom:.35rem;line-height:1.3}.chapter-desc{font-size:.78rem;color:rgba(255,255,255,.45);line-height:1.5;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;text-overflow:ellipsis;min-height:2.4em}.chapter-meta{display:flex;gap:.5rem;margin-top:.6rem}.chapter-pill{font-size:.62rem;font-weight:600;padding:.2rem .55rem;border-radius:12px;background:rgba(255,255,255,.06);color:rgba(255,255,255,.4);border:1px solid rgba(255,255,255,.08)}footer{text-align:center;padding:2rem;border-top:1px solid rgba(255,255,255,.06);position:relative;z-index:1}footer p{font-size:.75rem;color:rgba(255,255,255,.2)}.bonus-card{border-color:rgba(200,169,81,.35);background:rgba(200,169,81,.05)}.bonus-card:hover{border-color:var(--gold)}.bonus-pill{color:var(--gold)!important;border-color:rgba(200,169,81,.35)!important}@media(max-width:700px){.chapter-grid{grid-template-columns:1fr}.part-nav{flex-wrap:nowrap;overflow-x:auto;justify-content:flex-start}.part-nav a{flex-shrink:0}}@media (prefers-reduced-motion: reduce){.chapter-card{animation:none!important;opacity:1!important;transform:none!important;transition:none!important}}</style>';
