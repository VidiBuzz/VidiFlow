...
SEARCH_REGION = {
    "x_start": 0.70,  # Start search at 70% from left
    "y_start": 0.85,  # Start search at 85% from top
    "width": 0.30,  # Search 30% of width
    "height": 0.15,  # Search 15% of height
}


def create_vidismart_overlay(width, height):
    """Create VidiSmart branded overlay."""
    ...
    try:
        ...
        font_small = ImageFont.truetype(
            "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", int(height * 0.18)
        )
    except:
...
def process_image(image_path, template, dry_run=False):
    ...
    try:
        ...
        overlay = create_vidismart_overlay(logo_w, logo_h)

        # Paste overlay
        img.paste(overlay, (bounds[0], bounds[1]), overlay)
...
def process_video(video_path, template, dry_run=False):
    ...
    try:
        ...
        # Estimate logo size
        logo_w = int(original_w * 0.15)  # Typical logo is ~15% of width
        logo_h = int(logo_w * (template.shape[0] / template.shape[1]))

        if dry_run:
            return f"WOULD_PROCESS: {video_path.name} (video, {original_w}x{original_h}, logo at {x1},{y1})"

        # Create overlay
        overlay_img = create_vidismart_overlay(logo_w, logo_h)
        ...
        ffmpeg_cmd = [
            ...
            str(temp_overlay),
            "-filter_complex",
            f"[0:v][1:v]overlay={x1}:{y1}:enable='between(t,0,99999)'[outv]",
            "-map",
            "[outv]",
            "-map",
            "0:a?",
