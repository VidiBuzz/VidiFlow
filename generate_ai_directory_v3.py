#!/usr/bin/env python3
"""
Generate AI Companies Directory V3 - With Descriptions
Includes company descriptions scraped from websites
"""
...
try:
    ...
    descriptions = {}
    print("No descriptions file found")
...
def get_priority_score(business):
    ...
    if business.get('website'):
        score += 10
    if descriptions.get(business.get('name', '')):
        score += 15  # Boost companies with descriptions
...
for b in businesses:
    if is_valid_ai_company(b):
        ...
        if name in descriptions:
            b['description'] = descriptions[name]
...
# Count companies with descriptions
with_desc = len([b for b in unique_businesses if b.get('description')])
...
<body>
    ...
    <div class="stats">
        ...
        <div class="stat-box">
            <div class="number">{with_desc}</div>
    ...
    <nav class="nav">
        <div class="nav-inner">
            ...
            <a href="#saint-louis">Saint Louis</a>
        </div>
    </nav>
        ...
        for c in companies[:50]:  # Limit to 50 per type
            ...
            html += f'''
                <div class="company-card">
                    ...
                    {f'<div class="company-description">{desc}</div>' if desc else ''}
                    ...
                    <div class="company-contact">
                        ...
                        {f'<div class="company-info"><strong>Website:</strong> <a href="{website}" class="website" target="_blank">{website[:45]}{"..." if len(website) > 45 else ""}</a></div>' if website else ''}
                    </div>
                </div>
'''
        html += '            </div>\n'
...
html += '''
        ...
        <section id="by-region" class="section">
            <h2 class="section-title">Companies by Region</h2>
'''
...
html += f'''
    ...
    <footer class="footer">
        ...
        <p>Total Companies: {len(unique_businesses)} | With Descriptions: {with_desc} | Regions: {len(by_region)}</p>
        <p style="margin-top: 20px; color: #555;">Filtered to include only AI/ML consultants, coaches, and agencies.</p>
    </footer>
</body>
</html>
'''
...
# Write HTML file
with open('ai_consultants_directory_v3.html', 'w', encoding='utf-8') as f:
    f.write(html)

print(f"Generated ai_consultants_directory_v3.html")
print(f"Total unique AI companies: {len(unique_businesses)}")
print(f"Companies with descriptions: {with_desc}")
